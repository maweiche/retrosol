...
const Parcl = () => {
  ...
  async function getLastLiquiation() {
    const signatures = await connection.getSignaturesForAddress(
      new PublicKey("6dDCUve96a1Cqw3Zv34wfbCGwU77UEPe13953UdanTnT"),
      { limit: 1 },
    );
  ...
  const renderLPData = () => {
    return (
      <div className="flex flex-col justify-center items-center space-y-2 border-2 border-white p-4 gap-4">
        <p className="underline">Parcl Liquidity Pool Balance:</p>
        <p className="animate-pulse ">{usdcBalance} USDC</p>
        <p className="underline">Recent Contributions:</p>
  ...
  const renderMarketData = () => {
    return (
      <div className="flex flex-col justify-center items-center space-y-2 border-2 border-white p-4">
        <h4>Market Data</h4>
        <button
        ...
        >
          {!showStates && <div>Show States</div>}
          {showStates && <div>Hide States</div>}
        </button>
        {showStates && (
          <div className="flex flex-col justify-center items-center space-y-2">
            {!selectedState && (
                ...
                {states.map((state, index) => {
                  return (
                    <button
                      ...
                      style={{
                        backgroundColor:
                          state.abbreviation !== "NY" ? "gray" : "blue",
            ...
            {selectedState && cityData && (
              <div className="flex flex-col justify-center items-center space-y-2 m-6">
                <h1 className="animate-pulse text-4xl font-bold underline mb-4">
                  {selectedState.name}
                </h1>
                <button
                  className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                  onClick={() => {
                    setSelectedState(null), setCityData(null);
                  }}
                >
                  Back
                </button>
                    ...
                    {cityData.map((city, index) => {
                      return (
                        <button
                          ...
                          style={{
                            backgroundColor:
                              city.parcl_id !== 2900187 ? "gray" : "blue",
                            color:
                              city.parcl_id !== 2900187 ? "black" : "white",
                            borderRadius: "5px",
                            width: "fit-content",
                            padding: "5px",
                            whiteSpace: "nowrap",
                ...
                {!loading && marketSingles && (
                  <div className="flex flex-col justify-center items-center space-y-2 m-6">
                    <h1>Single Men: {marketSingles[0]}</h1>
                    <h1>Single Women: {marketSingles[1]}</h1>
                    <h1>Homes for Sale: {marketData}</h1>
                  </div>
                )}
                {loading && (
                  <div className="flex flex-col justify-center items-center space-y-2 m-6">
  ...
  useEffect(() => {
    ...
    const getStateMarkets = async () => {
      console.log(
        "pinging data point: ",
        `/api/parcl/markets?state=${selectedState.abbreviation}`,
      );
  ...
  useEffect(() => {
    ...
    const getCitySingles = async () => {
      console.log(
        "pinging data point: ",
        `/api/parcl/single?id=${selectedCity.parcl_id}`,
      );
      const response = await fetch(
        `/api/parcl/single?id=${selectedCity.parcl_id}`,
      );
    ...
    const getMarketData = async () => {
      console.log(
        "pinging data point: ",
        `/api/parcl/listings?id=${selectedCity.parcl_id}`,
      );
      const response = await fetch(
        `/api/parcl/listings?id=${selectedCity.parcl_id}`,
      );
